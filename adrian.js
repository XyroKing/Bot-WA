...
break
			...
			case 'igdl': {
			if (args.length == 0) return newReply(`Example: ${prefix + command} https://www.instagram.com/p/Cs5gcYCvmTw/?igshid=NTc4MTIwNjQ2YQ==`)
			if (!isPremium && global.db.data.users[m.sender].limit < 1) return newReply(mess.endLimit) // respon ketika limit habis
                db.data.users[m.sender].limit -= 10
			newReply(mess.wait)
		...
		case 'brainly':{
			...
			newReply(mess.wait)
			axios.get(`https://api.lolhuman.xyz/api/openai?apikey=${apikey}&text=${encodeURIComponent(full_args)}&user=${m.sender.split('@')[0]}`).then(({ data }) => {
				newReply(data.result)
			})
			break
